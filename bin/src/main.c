...
int multi_pkts(const u_char *pkts, u_int total_len, char *server, char *clientip, u_short pkt_id);
int parse_pkt(u_char *pkt, u_int len, char *server, char *clientip, u_short pkt_id);
...
int main(int argc, char *argv[])
{
   ...
   printf("Listening for MySQL traffic on interface %s %s\n", dev, filter_exp);
...
void show_help(void)
{
   printf("Usage: main [OPTIONS] INTERFACE\n\n");
   printf("OPTIONS:\n");
...
void proc_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
   ...
   // single direction
   char client[50], server[50];
   strncpy (client, inet_ntoa(ip->ip_src), sizeof (client));
   strncpy (server, inet_ntoa(ip->ip_dst), sizeof (server));

   if(size_mysql > 0 || op.tcp_ctrl) {
      if(ntohs(tcp->th_sport) == op.port_n) {
          printf ("QUERY_DONE ID=\"%d\"\n", htons (tcp->th_dport));
          fflush (stdout);
...
//         printf("%s.%d > server: ", inet_ntoa(ip->ip_src), ntohs(tcp->th_sport));
   ...
   if(size_mysql > 4) {
      ...
      retval = multi_pkts(mysql, size_mysql, server, client, htons(tcp->th_sport));
   }
   else
...
//      if(!op.no_myhdrs) printf("ID %u len %u ", m->pkt_id, m->pkt_length);
      ...
      if(m->pkt_length) {
         ...
         retval = parse_pkt(buff, m->pkt_length, server, clientip, pkt_id);
      }
//      else
...
int parse_pkt(u_char *pkt, u_int len, char *server, char *clientip, u_short pkt_id)
{
   ...
   if(*pkt == 0xFF) { // Error
      pkt_error(pkt, len, server, clientip, pkt_id);
...
DETERMINE_PKT:
      if(tag->current_origin == ORIGIN_SERVER) {
         ...
         if((*ha)(pkt, len, server, clientip, pkt_id) == PKT_HANDLED) {
...
//   if(op.state) printf("state '%s' ", state_name[state]);
   ...
   if(num_events == 1) {
      ...
      if((*ha)(pkt, len, server, clientip, pkt_id))
...
/*
  ...
  The following functions provides the means by which sniffer maintains
