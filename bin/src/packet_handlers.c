...
extern struct st_options op;      // Declared in tag.h
extern tag_id *tag;               // Declared in tag.h
...
// MySQL end pkt
int pkt_end(u_char *pkt, u_int len, char *server, char *client, u_short pkt_id)
{
...
int pkt_error(u_char *pkt, u_int len, char *server, char *client, u_short pkt_id)
{
...
int pkt_com_x(u_char *pkt, u_int len, char *server, char *client, u_short pkt_id)
{
...
int pkt_com_x_string(u_char *pkt, u_int len, char *server, char *client, u_short pkt_id)
{
   if(len == 1 || *pkt > COM_END) return PKT_WRONG_TYPE;
   ...
   printf("\nID=\"%d\" SERVER=\"%s\" CLIENT=\"%s\" TYPE=\"COM_%s\": %s\n", pkt_id, server, client, command_name[*pkt], get_arg(pkt, len));
   fflush(stdout);

   return PKT_HANDLED;
}

int pkt_com_x_int(u_char *pkt, u_int len, char *server, char *client, u_short pkt_id)
{
   ...
   printf("ID=\"%d\" COM_%s: ", pkt_id, command_name[*pkt]);
   *pkt++;
...
int pkt_string(u_char *pkt, u_int len, char *server, char *client, u_short pkt_id)
{
...
int pkt_binary_row(u_char *pkt, u_int len, char *server, char *client, u_short pkt_id)
{
   ...
   return pkt_row(row, row_len, server, client, pkt_id);
}
